...
counties_included_combined <- 
  full_join(counties_included_total, counties_included_monthly, by = "county_geoid_coalesced") %>% 
   mutate(source = case_when(
    source.y == "Suppressed in County-Month" & source.x == "Suppressed in County-Total" ~ "Data suppressed", 
    source.y == "Suppressed in County-Month" & source.x == "Included only in County-Total" ~ "Included only in County-Total",
    is.na(source.y) & source.x == "Suppressed in County-Total" ~ "Data suppressed",
...
# pal <- colorFactor(palette = c("yellow", "#b69ee2", "#7ebcef", "lightgray"), domain = map_data$source)
bupu <- brewer.pal(n = 4, name = "BuPu") %>% rev()
pal <- colorFactor(palette = bupu, domain = map_data$source)
